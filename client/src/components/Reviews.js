...
//=====================================================================================================================================
  componentDidMount() {
    ...
    this.state.reviews.data.forEach(review => {
      ...
      .then(data => data.json())
      .then(data => {
        let obj = {content: review.attributes.content, userName: data.data.attributes.name, avatar: data.data.attributes.avatar, reviewId: review.id}
        userInfo.push(obj)
        if (userInfo.length === this.state.reviews.data.length) {
          let sortedUserInfo = userInfo.sort((a, b) => a.reviewId - b.reviewId)
    ...
    if (this.state.reviews.links.prev) {
      ...
      .then(data => {
        ...
        data.data.forEach(review => {
          ...
          .then(nestedData => {
            let obj = {content: review.attributes.content, userName: nestedData.data.attributes.name, avatar: nestedData.data.attributes.avatar, reviewId: review.id}
            userInfo.push(obj)
